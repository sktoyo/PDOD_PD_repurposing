...
def get_phenotype_to_gene_mapping_for_chemicals(file_name, drug_name, anatomies_accepted, go_id_to_genes=None):
    ...
    anatomies_all = set()
    phenotypes_all = set()
    for chemical, values in key_to_values.items():
        for vals in values:
...
def parse_CTD():
    ...
    for disease, values in disease_to_values.items():                                               
        genes = set()
        genes_direct = set()
...
def get_inference_scores_for_disease(dir_name, disease_to_values, disease_name):
    ...
    for disease, values in disease_to_values.items():                                               
        if disease.startswith(disease_name):                                                           
            ...
            for gene, is_direct in gene_to_direct.items():
...
def get_nc_scores_for_disease(dir_name, disease):
    ...
    for gene, score in node_to_score.items():
        in_ctd = 0
        is_direct = 0
        if gene in genes:                                               
            in_ctd = 1
