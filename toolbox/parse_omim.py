...
def get_disease_gene_mapping(dir_name):
    ...
    disease_to_genes = {}
    disease_to_loci = {}
    for disorder, genes in disorder_to_genes.items():
        ...
        for disorder_word in disorder_words:
            for separator in ("-", "/"):
                ...
                if len(words) > 1 and words[1] in diseases:
                    ...
                    a = disease_to_loci.setdefault(disease, set())
                    disease_to_loci[disease] = a.union(disorder_to_loci[disorder]) 
    print(list(disease_to_genes.keys())[:10])
    ...
    print(list(selected_disease_to_loci.keys())[:10])
    print(list(selected_disease_to_loci.values())[:10])
    print(len(selected_disease_to_genes))

    diseases = list(selected_disease_to_genes.keys())
    diseases.sort()
    print(diseases)
...
def get_candidates_by_loci_matching(disease_to_genes, disease_to_loci, dir_name, uniprot_dir_name, via="biomart"):
    ...
    if via == "biomart":
        ...
        for disease, vals in disease_to_loci.items():
            for loci in vals:
        ...
        for disease, vals in disease_to_loci.items():
            for loci in vals:
                for gene, uniprot_loci in gene_to_loci.items():
    ...
    for disease, vals in disease_to_candidates.items():
