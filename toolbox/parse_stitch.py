...
def get_drugbank_to_targets(pubchem_to_drugbank_ids, pubchem_to_target_to_score, cutoff):
    ...
    for cid, drugbank_ids in pubchem_to_drugbank_ids.items():
        if cid in pubchem_to_target_to_score:
            for target, score in pubchem_to_target_to_score[cid].items():
                if score >= cutoff:
...
def get_inchikey_to_targets(pubchem_to_target_to_score, pubchem_to_inchi, cutoff):
    inchi_to_targets = {}
    for cid, targets in pubchem_to_target_to_score.items():
        if cid not in pubchem_to_inchi:
            continue
        for geneid, score in target_to_score.items():
            if score >= cutoff:
