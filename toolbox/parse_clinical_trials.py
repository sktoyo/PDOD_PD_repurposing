...
def output_data(base_dir, file_name):
    ...
    print(len(drug_to_ctids), list(drug_to_ctids.items())[:5])
    ctid_to_conditions = get_ctid_to_conditions(base_dir)
    print(len(ctid_to_conditions), list(ctid_to_conditions.items())[:5])
    ctid_to_values = get_ctid_to_details(base_dir) 
    print(len(ctid_to_values), list(ctid_to_values.items())[:5])
    f = open(file_name, 'w')
    f.write("Drug\tClinical trial Id\tPhase\tStatus\tFDA regulated\tWhy stopped\tResults date\tConditions\n")

    for drug, ctids in drug_to_ctids.items():
        for ctid in ctids:
            values = [ drug, ctid ]
...
def get_disease_specific_drugs(drug_to_diseases, phenotype_to_mesh_id):
    ...
    for phenotype, mesh_id in list(phenotype_to_mesh_id.items()):
...
def get_drug_disease_mapping(base_dir, selected_drugs, name_to_drug, synonym_to_drug, mesh_id_to_name, mesh_id_to_name_with_synonyms, dump_file):
    ...
    mesh_name_to_id = {}
    for mesh_id, names in mesh_id_to_name_with_synonyms.items():
        for name in names:
    ...
    interventions = reduce(lambda x,y: x|y, list(ctid_to_conditions.values()))
    for intervention in interventions:
        ...
        for mesh_name, dui in mesh_name_to_id.items():
    ...
    for drug, ctids in drug_to_ctids.items():
        ...
        for phenotype, val in phenotype_to_count.items(): 
            dui = mesh_name_to_id[phenotype]
...
def get_frequent_interventions(drug_to_interventions):
    condition_to_count = {}
    for drug, interventions in drug_to_interventions.items():
        for condition in interventions:
            if condition in condition_to_count:
                ...
                condition_to_count[condition] = 1
    values = []
    for condition, count in condition_to_count.items():
