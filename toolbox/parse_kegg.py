...
def get_drug_disease_mapping(drugbank_to_kegg_id, dump_file):
    ...
    #flag = False
    for drugbank_id, kegg_id in drugbank_to_kegg_id.items():
...
def get_kegg_drug_mesh_mapping(kegg_ids = None):
    ...
    print(len(kegg_drug_to_diseases), list(kegg_drug_to_diseases.items())[:5])
    # Get corresponding mesh ids of diseases 
    diseases = reduce(lambda x,y: x | y, list(kegg_drug_to_diseases.values()))
    ...
    print(len(kegg_drug_to_phenotype_and_mesh_ids), list(kegg_drug_to_phenotype_and_mesh_ids.items())[:5])
...
def output_interactions(out_file):
    ...
    reaction_ids = list(reaction_to_values.keys())
