...
def get_sider_info(label_file, side_effect_file, meddra_side_effect_file, dump_file):
    if os.path.exists(dump_file):
        pubchem_to_siderids, siderid_to_descriptions, pubchem_to_names = pickle.load(open(dump_file))
    ...
    pickle.dump((pubchem_to_siderids, siderid_to_descriptions, pubchem_to_names), open(dump_file, 'w')) 
    return pubchem_to_siderids, siderid_to_descriptions, pubchem_to_names
...
def parse_meddra_side_effects(meddra_side_effect_file):
    ...
    for name, values in id_to_values.items():
        name = name.lower()
        for val in values:
...
def parse_side_effects(side_effect_file, label_file):
    ...
    for label, cids in label_to_specific_cids.items():
        for cid in cids:
            if cid == "":
                continue
            cid = "%s" % abs(int(cid))
...
def parse_raw_side_effects(side_effect_file):
    ...
    for label, values in id_to_values.items():
        words = label.split(",")
        for word in words:
            for val in values:
