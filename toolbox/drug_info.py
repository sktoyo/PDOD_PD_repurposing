...
import os, pickle, numpy, random #, time,
...
def get_drug_targets(parameters, parser=None, id_type="geneid"):
    ...
    uniprot_ids = reduce(lambda x,y: x|y, list(drug_to_uniprots.values()))
    if id_type == "uniprot":
...
def get_drug_similarity(drug_to_values, method="target", network=None, dump_file=None):
    ...
    if dump_file is not None and os.path.exists(dump_file):
        drug_to_drug_similarity = pickle.load(open(dump_file))
    ...
    drugs = list(drug_to_values.keys())
    ...
    if dump_file is not None:
        pickle.dump(drug_to_drug_similarity, open(dump_file,'w'))
    return drug_to_drug_similarity
...
def get_chemical_similarity_based_target_predictions(parameters, smiles_list, cutoff=0.9, method="fishers"):
    ...
    all_smiles = reduce(lambda x,y: x|y, list(geneid_to_smiles_strings.values()))
...
def get_side_effect_targets(parameters, source = "sider"):
    ...
    if os.path.exists(dump_file):
        side_effect_to_targets = pickle.load(open(dump_file))
    ...
    pickle.dump(side_effect_to_targets, open(dump_file,'w'))
    return side_effect_to_targets
...
def get_side_effect_targets_fishers(drug_to_geneids, drug_to_side_effects, cutoff=0.2, correct_pvalues=True): # min_n_drug=5, 
    ...
    for side_effect, drugs_se in side_effect_to_drugs.items():
        ...
        if correct_pvalues:
            pvalues_new = stat_utilities.correct_pvalues_for_multiple_testing(list(zip(*values))[0])
...
def get_drug_side_effects(parameters, source=None):
    ...
    if os.path.exists(dump_file):
        drugbank_id_to_side_effects = pickle.load(open(dump_file))
    ...
    pickle.dump(drugbank_id_to_side_effects, open(dump_file,'w'))
    return drugbank_id_to_side_effects
...
def get_offsides(parameters):
    ...
    for cid, values in cid_to_values.items():
        side_effects |= set([x.lower() for x in zip(*values)[0]])
...
def get_nugent(parameters):
    ...
    for name, side_effect_to_count in name_to_side_effect_counts.items():
        ...
        for side_effect, count in side_effect_to_count.items():
            ...
            if count >= 5:
                drug_to_side_effects.setdefault(drug, set()).add(side_effect)
    print(len(drug_to_side_effects), list(drug_to_side_effects.items())[:3])
    print("Drugs not in drugbank mapping:", len(not_in_db)) #, not_in_db
    return drug_to_side_effects 
